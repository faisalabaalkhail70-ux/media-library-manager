...
class RenameService:
    ...
    def _format_template(self, row: dict, template: str) -> str:
        ...
        values = {
            ...
            "{Season:02}": f'{int(row["season_number"]):02d}' if row.get("season_number") is not None else "",
            "{Episode:02}": f'{int(row["episode_number"]):02d}' if row.get("episode_number") is not None else "",
            "{Ext}": ext,
        }
